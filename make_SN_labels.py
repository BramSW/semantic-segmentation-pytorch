...
def make_normal_labels(mat_files_dir, dst_lab_files_dir, vocab_file):
    ...
    codebook = vocab['vocabs'][0][0]['normals'][0][0]
    codebook = codebook.astype(np.float64)
    normal_info = load_pickle('/data/bw462/nyu/surfacenormal_metadata/all_normals.pklz')
    assert NUM_NORMALS_CLASSES == codebook.shape[0]
    # whether to create Left-Right Flipped ground truth. This is beta tested only.
    flip = False # flip = True is beta
...
def main():
    ...
    image_dir = '/data/bw462/nyu' # directory containing NYU images
    splits_dir = '/data/bw462/nyu/surfacenormal_metadata' # directory containing train/test splits text files
    mat_files_dir = '/data/bw462/nyu/' # directory containing .mat files ground truth

    # The following directories are populated with data
    dst_normals_dir = 'tmp' # dst dir where normal files will be created
